...
def dytimu(nj=2, sm=200):  # 打印题目，nj年级默认2，sm数目默认200
    ...
    for x in range(xh):
        ...
        s3=[]       #答案
        for s1 in suanshi:
            s2.append(s1[0: s1.rfind('=')+1])
            s3.append(s1[s1.rfind('=')+1:])

            """
            Value 's1' is unsubscriptable 下标超界，可以忽略
            """
        wb.sheets['sheet1'].range('B'+str(x+1)).value = s2  #写入不带答案算式
        wb.sheets['sheet1'].range('H'+str(x+1)).value = s3  #打印答案
        x1 += 5
        x2 += 5
